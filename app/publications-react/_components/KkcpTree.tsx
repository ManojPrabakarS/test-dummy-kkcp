// @ts-nocheck
import { KkcpHeader } from "@/components/KkcpHeader";
import { KkcpFooter } from "@/components/KkcpFooter";
// @ts-nocheck
// Auto-generated DOM→JSX emit of the mirrored page.
// Real React tree (no iframe, no dangerouslySetInnerHTML). Source class names + inline styles
// preserved; the source CSS lives scoped in publications-inline.css. Asset paths absolute to /research/.
/* eslint-disable */
export function KkcpTree() {
  return (
    <div
      className="wp-singular page-template-default page page-id-5879 wp-theme-KKCP gsap-enable elementor-default elementor-template-full-width elementor-kit-14 elementor-page elementor-page-5879 e--ua-blink e--ua-mac e--ua-webkit"
      data-elementor-device-mode={"desktop"}
      data-aos-easing={"ease"}
      data-aos-duration={"800"}
      data-aos-delay={"0"}
      style={{}}
    >
      {" "}
      <div
        id="site-preloader"
        className="kkcp-preloader"
        style={{ display: "none" }}
      >
        {" "}
        <div className="loader-container">
          {" "}
          <div className="loader-icon">
            {" "}
            <img src="/research/assets/0025__Asset-2-11.webp" alt="KKCP" />{" "}
          </div>{" "}
        </div>
      </div>{" "}
      <div id="kkcp-page" className="kkcp-page-wrapper">
        {" "}
        <KkcpHeader />{" "}
        <main id="kkcp-content" className="kkcp-content-wrapper">
          {" "}
          <div
            data-elementor-type={"wp-page"}
            data-elementor-id={"5879"}
            className="elementor elementor-5879"
          >
            <div
              data-aos-once={"true"}
              className="elementor-element elementor-element-b50ea0f e-flex e-con-boxed e-con e-parent e-lazyloaded"
              data-id={"b50ea0f"}
              data-element_type={"container"}
              data-e-type={"container"}
              data-settings={'{"background_background":"classic"}'}
            >
              <div className="e-con-inner">
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-546df21 elementor-widget elementor-widget-rstb-breadcrumb"
                  data-id={"546df21"}
                  data-element_type={"widget"}
                  data-e-type={"widget"}
                  data-widget_type={"rstb-breadcrumb.default"}
                >
                  {" "}
                  <div className="rstb-breadcrumb">
                    <span property="itemListElement">
                      <a
                        property="item"
                        title="Go to KKCP."
                        href="/"
                        className="home"
                      >
                        <span property="name">Home</span>
                      </a>
                      <meta property="position" content="1" />
                    </span>
                    <span className="item-separator">
                      <svg
                        className="e-font-icon-svg e-fas-angle-double-right"
                        viewBox="0 0 448 512"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path d="M224.3 273l-136 136c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-33.9l96.4-96.4-96.4-96.4c-9.4-9.4-9.4-24.6 0-33.9L54.3 103c9.4-9.4 24.6-9.4 33.9 0l136 136c9.5 9.4 9.5 24.6.1 34zm192-34l-136-136c-9.4-9.4-24.6-9.4-33.9 0l-22.6 22.6c-9.4 9.4-9.4 24.6 0 33.9l96.4 96.4-96.4 96.4c-9.4 9.4-9.4 24.6 0 33.9l22.6 22.6c9.4 9.4 24.6 9.4 33.9 0l136-136c9.4-9.2 9.4-24.4 0-33.8z" />
                      </svg>
                    </span>
                    <span property="itemListElement">
                      <span
                        property="name"
                        className="post post-page current-item"
                      >
                        Publications
                      </span>
                      <meta property="url" content="/research/" />
                      <meta property="position" content="2" />
                    </span>{" "}
                  </div>
                </div>
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-be2c5eb elementor-widget elementor-widget-rstb-page-title"
                  data-id={"be2c5eb"}
                  data-element_type={"widget"}
                  data-e-type={"widget"}
                  data-widget_type={"rstb-page-title.default"}
                >
                  <h1 className="rstb-page-title">Publications</h1>
                </div>
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-665d971 elementor-widget elementor-widget-rs-divider"
                  data-id={"665d971"}
                  data-element_type={"widget"}
                  data-e-type={"widget"}
                  data-widget_type={"rs-divider.default"}
                >
                  {" "}
                  <div className="rs-divider dot-enable">
                    {" "}
                    <span> </span>{" "}
                  </div>
                </div>
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-c42c095 elementor-widget__width-initial elementor-widget elementor-widget-text-editor"
                  data-id={"c42c095"}
                  data-element_type={"widget"}
                  data-e-type={"widget"}
                  data-widget_type={"text-editor.default"}
                >
                  <p>
                    Education goes beyond textbooks and classrooms. We believe
                    in empowering students to explore their passions challenge
                    conventions.
                  </p>
                </div>
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-9ee44a9 elementor-absolute gsap-move-yes down-90 start-10 elementor-hidden-mobile elementor-widget elementor-widget-rs-image"
                  data-id={"9ee44a9"}
                  data-element_type={"widget"}
                  data-e-type={"widget"}
                  data-settings={'{"_position":"absolute"}'}
                  data-widget_type={"rs-image.default"}
                  style={{ transform: "translate(0px, 0px)" }}
                >
                  {" "}
                  <div className="rs-image">
                    {" "}
                    <img
                      decoding="async"
                      className="rs-multi-image  reverse- blend_unset"
                      src="/research/assets/0049__bnr-arrow-1-1.webp"
                      alt="bnr-arrow-1-1"
                    />{" "}
                  </div>
                </div>
              </div>
            </div>
            <div
              data-aos-once={"true"}
              className="elementor-element elementor-element-fc7a512 e-flex e-con-boxed e-con e-parent e-lazyloaded"
              data-id={"fc7a512"}
              data-element_type={"container"}
              data-e-type={"container"}
              data-settings={'{"background_background":"classic"}'}
            >
              <div className="e-con-inner">
                <div
                  data-aos-once={"true"}
                  className="elementor-element elementor-element-2e679d0 e-con-full e-flex e-con e-child"
                  data-id={"2e679d0"}
                  data-element_type={"container"}
                  data-e-type={"container"}
                >
                  <div
                    data-aos-once={"true"}
                    className="elementor-element elementor-element-c502feb e-con-full e-flex e-con e-child"
                    data-id={"c502feb"}
                    data-element_type={"container"}
                    data-e-type={"container"}
                  >
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-3093adc elementor-widget elementor-widget-heading"
                      data-id={"3093adc"}
                      data-element_type={"widget"}
                      data-e-type={"widget"}
                      data-widget_type={"heading.default"}
                    >
                      <h3 className="elementor-heading-title elementor-size-default">
                        Research Highlights
                      </h3>
                    </div>
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-cff5127 elementor-widget elementor-widget-text-editor"
                      data-id={"cff5127"}
                      data-element_type={"widget"}
                      data-e-type={"widget"}
                      data-widget_type={"text-editor.default"}
                    >
                      <p>
                        We actively support research at every level, from
                        undergraduate exploration to advanced doctoral inquiry,
                        ensuring opportunities for hands-on learning, discovery
                        and leadership. Through publications, conferences, and
                        global collaborations, our researchers contribute to
                        international academic.
                      </p>
                    </div>
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-77ea7ce elementor-widget elementor-widget-rs-research"
                      data-id={"77ea7ce"}
                      data-element_type={"widget"}
                      data-e-type={"widget"}
                      data-widget_type={"rs-research.default"}
                    >
                      {" "}
                      <div className="rs-research style-one">
                        {" "}
                        <div className="single-item">
                          {" "}
                          <div className="content-wrapper">
                            {" "}
                            <div className="content-inner">
                              {" "}
                              <div className="publish-date">
                                {" "}
                                <i className="ri-calendar-line"></i>
                                November 11, 2025{" "}
                              </div>
                              <h4 className="title">
                                {" "}
                                <a href="/researches/integrative-research-in-applied-science-and-engineering-innovation/">
                                  Integrative Research in Applied Science and
                                  Engineering Innovation
                                </a>
                              </h4>{" "}
                              <div className="researcher_label">
                                By 4 Researchers
                              </div>{" "}
                            </div>{" "}
                            <a
                              className="rs-button style-default icon-anim-flip-right text-anim-flip-top"
                              href="/wp-content/uploads/sites/2/2025/12/semester_exam_schedule_notice.pdf"
                            >
                              {" "}
                              <span className="button-icon">
                                {" "}
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>{" "}
                              </span>{" "}
                              <span
                                className="button-text"
                                data-text={"Download PDF"}
                              >
                                Download PDF{" "}
                              </span>{" "}
                            </a>{" "}
                          </div>{" "}
                          <a
                            className="thumb-wrapper"
                            href="/researches/integrative-research-in-applied-science-and-engineering-innovation/"
                          >
                            <img
                              loading="lazy"
                              decoding="async"
                              width="770"
                              height="660"
                              src="/research/assets/0056__acc-16-min.webp"
                              className="attachment-full size-full wp-post-image"
                              alt=""
                              srcSet="/research/assets/0056__acc-16-min.webp 770w, /research/assets/0076__acc-16-min-300x257.webp 300w, /research/assets/0077__acc-16-min-768x658.webp 768w"
                              sizes="(max-width: 770px) 100vw, 770px"
                            />{" "}
                          </a>{" "}
                        </div>{" "}
                        <div className="single-item">
                          {" "}
                          <div className="content-wrapper">
                            {" "}
                            <div className="content-inner">
                              {" "}
                              <div className="publish-date">
                                {" "}
                                <i className="ri-calendar-line"></i>
                                September 11, 2025{" "}
                              </div>
                              <h4 className="title">
                                {" "}
                                <a href="/researches/transformative-applied-science-and-engineering-research-projects/">
                                  Transformative Applied Science and Engineering
                                  Research Projects
                                </a>
                              </h4>{" "}
                              <div className="researcher_label">
                                By 4 Researchers
                              </div>{" "}
                            </div>{" "}
                            <a
                              className="rs-button style-default icon-anim-flip-right text-anim-flip-top"
                              href="/wp-content/uploads/sites/2/2025/12/semester_exam_schedule_notice.pdf"
                            >
                              {" "}
                              <span className="button-icon">
                                {" "}
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>{" "}
                              </span>{" "}
                              <span
                                className="button-text"
                                data-text={"Download PDF"}
                              >
                                Download PDF{" "}
                              </span>{" "}
                            </a>{" "}
                          </div>{" "}
                          <a
                            className="thumb-wrapper"
                            href="/researches/transformative-applied-science-and-engineering-research-projects/"
                          >
                            <img
                              loading="lazy"
                              decoding="async"
                              width="770"
                              height="660"
                              src="/research/assets/0060__acc-14-min.webp"
                              className="attachment-full size-full wp-post-image"
                              alt=""
                              srcSet="/research/assets/0060__acc-14-min.webp 770w, /research/assets/0078__acc-14-min-300x257.webp 300w, /research/assets/0079__acc-14-min-768x658.webp 768w"
                              sizes="(max-width: 770px) 100vw, 770px"
                            />{" "}
                          </a>{" "}
                        </div>{" "}
                        <div className="single-item">
                          {" "}
                          <div className="content-wrapper">
                            {" "}
                            <div className="content-inner">
                              {" "}
                              <div className="publish-date">
                                {" "}
                                <i className="ri-calendar-line"></i>
                                August 14, 2025{" "}
                              </div>
                              <h4 className="title">
                                {" "}
                                <a href="/researches/research-innovation-academic-inquiry-global-impact/">
                                  Research Innovation, Academic Inquiry &amp;
                                  Global Impact
                                </a>
                              </h4>{" "}
                              <div className="researcher_label">
                                By 4 Researchers
                              </div>{" "}
                            </div>{" "}
                            <a
                              className="rs-button style-default icon-anim-flip-right text-anim-flip-top"
                              href="/wp-content/uploads/sites/2/2025/12/semester_exam_schedule_notice.pdf"
                            >
                              {" "}
                              <span className="button-icon">
                                {" "}
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  viewBox="0 0 20 20"
                                >
                                  <path d="M13.6306 8.65779V4.87288C13.6306 4.76456 13.5806 4.66444 13.5098 4.58526L9.26635 0.129258C9.18709 0.0459374 9.07455 0 8.96205 0H2.23421C0.992107 0 0 1.01289 0 2.25507V14.9771C0 16.2193 0.992107 17.2156 2.23418 17.2156H7.54483C8.54932 18.8829 10.3752 20 12.4551 20C15.6147 20 18.195 17.4322 18.195 14.2684C18.1993 11.5048 16.215 9.19553 13.6306 8.65783V8.65779ZM9.37897 1.4632L12.2259 4.46019H10.3793C9.82908 4.46019 9.37893 4.00589 9.37893 3.4557L9.37897 1.4632ZM2.23418 16.3818C1.45476 16.3818 0.833748 15.7565 0.833748 14.9771V2.25507C0.833748 1.4714 1.45476 0.833748 2.23418 0.833748H8.54522V3.45562C8.54522 4.46851 9.36643 5.2939 10.3793 5.2939H12.7969V8.54936C12.6719 8.54522 12.5719 8.53268 12.4635 8.53268C11.0087 8.53268 9.67068 9.09131 8.66194 9.9667H3.36808C3.13874 9.9667 2.95121 10.1542 2.95121 10.3834C2.95121 10.6128 3.13871 10.8003 3.36808 10.8003H7.88663C7.59061 11.2172 7.34463 11.634 7.15303 12.0926H3.36804C3.13871 12.0926 2.95117 12.2801 2.95117 12.5094C2.95117 12.7386 3.13867 12.9263 3.36804 12.9263H6.88198C6.77776 13.3432 6.72358 13.8058 6.72358 14.2685C6.72358 15.0187 6.86944 15.7608 7.13206 16.3861H2.23418V16.3818ZM12.4594 19.1705C9.75826 19.1705 7.56147 16.9737 7.56147 14.2726C7.56147 11.5715 9.75404 9.37467 12.4594 9.37467C15.1646 9.37467 17.3572 11.5715 17.3572 14.2726C17.3572 16.9737 15.1605 19.1705 12.4594 19.1705Z" />
                                  <path d="M3.36805 8.71188H7.59059C7.81992 8.71188 8.00746 8.52423 8.00746 8.29501C8.00746 8.06571 7.81996 7.87817 7.59059 7.87817H3.36805C3.13871 7.87817 2.95117 8.06567 2.95117 8.29501C2.95117 8.52423 3.13867 8.71188 3.36805 8.71188ZM14.5935 14.1849L12.8804 16.0315V11.4797C12.8804 11.2504 12.6927 11.0628 12.4635 11.0628C12.2342 11.0628 12.0466 11.2503 12.0466 11.4797V16.0315L10.3209 14.1849C10.1625 14.0183 9.89574 14.0058 9.72898 14.1642C9.56219 14.3225 9.54969 14.5852 9.7082 14.7519L12.1467 17.3738C12.2259 17.4572 12.3342 17.5072 12.451 17.5072C12.5677 17.5072 12.676 17.4572 12.7552 17.3738L15.198 14.752C15.3564 14.5852 15.348 14.3184 15.1812 14.1642C15.0104 14.0058 14.752 14.0183 14.5935 14.1849Z" />
                                </svg>{" "}
                              </span>{" "}
                              <span
                                className="button-text"
                                data-text={"Download PDF"}
                              >
                                Download PDF{" "}
                              </span>{" "}
                            </a>{" "}
                          </div>{" "}
                          <a
                            className="thumb-wrapper"
                            href="/researches/research-innovation-academic-inquiry-global-impact/"
                          >
                            <img
                              loading="lazy"
                              decoding="async"
                              width="770"
                              height="660"
                              src="/research/assets/0061__acc-4-min.webp"
                              className="attachment-full size-full wp-post-image"
                              alt=""
                              srcSet="/research/assets/0061__acc-4-min.webp 770w, /research/assets/0080__acc-4-min-300x257.webp 300w, /research/assets/0081__acc-4-min-768x658.webp 768w"
                              sizes="(max-width: 770px) 100vw, 770px"
                            />{" "}
                          </a>{" "}
                        </div>{" "}
                      </div>
                    </div>
                  </div>
                  <div
                    data-aos-once={"true"}
                    className="elementor-element elementor-element-ccf514f e-con-full e-flex e-con e-child"
                    data-id={"ccf514f"}
                    data-element_type={"container"}
                    data-e-type={"container"}
                  >
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-8bac476 elementor-widget elementor-widget-rs-image"
                      data-id={"8bac476"}
                      data-element_type={"widget"}
                      data-e-type={"widget"}
                      data-widget_type={"rs-image.default"}
                    >
                      {" "}
                      <div className="rs-image">
                        {" "}
                        <img
                          decoding="async"
                          className="rs-multi-image  reverse- blend_unset"
                          src="/research/assets/0059__research-img1-min.webp"
                          alt="research-img1-min"
                        />{" "}
                      </div>
                    </div>
                  </div>
                  <div
                    data-aos-once={"true"}
                    className="elementor-element elementor-element-527aee6 e-con-full e-flex e-con e-child"
                    data-id={"527aee6"}
                    data-element_type={"container"}
                    data-e-type={"container"}
                  >
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-e345703 elementor-widget elementor-widget-heading"
                      data-id={"e345703"}
                      data-element_type={"widget"}
                      data-e-type={"widget"}
                      data-widget_type={"heading.default"}
                    >
                      <h3 className="elementor-heading-title elementor-size-default">
                        Research Focus Areas
                      </h3>
                    </div>
                  </div>
                  <div
                    data-aos-once={"true"}
                    className="elementor-element elementor-element-c243de2 e-con-full e-flex e-con e-child"
                    data-id={"c243de2"}
                    data-element_type={"container"}
                    data-e-type={"container"}
                    data-settings={'{"background_background":"classic"}'}
                  >
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-92a11fc e-con-full e-flex e-con e-child"
                      data-id={"92a11fc"}
                      data-element_type={"container"}
                      data-e-type={"container"}
                    >
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-486d1cb elementor-widget elementor-widget-heading"
                        data-id={"486d1cb"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"heading.default"}
                      >
                        <h6 className="elementor-heading-title elementor-size-default">
                          1. Science &amp; Technology
                        </h6>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-d7cb801 elementor-icon-list--layout-traditional elementor-list-item-link-full_width elementor-widget elementor-widget-icon-list"
                        data-id={"d7cb801"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-list.default"}
                      >
                        <ul className="elementor-icon-list-items">
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Completed secondary education
                              (HSC/A-Level/Equivalent).
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Minimum GPA/grade requirements as set by the
                              university.
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              For graduate programs: a recognized bachelor’s
                              degree with required CGPA.
                            </span>
                          </li>
                        </ul>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-d0664e9 elementor-widget elementor-widget-heading"
                        data-id={"d0664e9"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"heading.default"}
                      >
                        <h6 className="elementor-heading-title elementor-size-default">
                          2. Health &amp; Medicine
                        </h6>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-5514aea elementor-icon-list--layout-traditional elementor-list-item-link-full_width elementor-widget elementor-widget-icon-list"
                        data-id={"5514aea"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-list.default"}
                      >
                        <ul className="elementor-icon-list-items">
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Completed secondary education
                              (HSC/A-Level/Equivalent).
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Minimum GPA/grade requirements as set by the
                              university.
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              For graduate programs: a recognized bachelor’s
                              degree with required CGPA.
                            </span>
                          </li>
                        </ul>
                      </div>
                    </div>
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-efbb1d5 e-con-full e-flex e-con e-child"
                      data-id={"efbb1d5"}
                      data-element_type={"container"}
                      data-e-type={"container"}
                    >
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-34810c7 elementor-widget elementor-widget-heading"
                        data-id={"34810c7"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"heading.default"}
                      >
                        <h6 className="elementor-heading-title elementor-size-default">
                          3. Engineering &amp; Innovation
                        </h6>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-842fdcb elementor-icon-list--layout-traditional elementor-list-item-link-full_width elementor-widget elementor-widget-icon-list"
                        data-id={"842fdcb"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-list.default"}
                      >
                        <ul className="elementor-icon-list-items">
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Completed secondary education
                              (HSC/A-Level/Equivalent).
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Minimum GPA/grade requirements as set by the
                              university.
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              For graduate programs: a bachelor’s degree with
                              required CGPA.
                            </span>
                          </li>
                        </ul>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-a038242 elementor-widget elementor-widget-heading"
                        data-id={"a038242"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"heading.default"}
                      >
                        <h6 className="elementor-heading-title elementor-size-default">
                          4. Business &amp; Economics
                        </h6>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-e9064d9 elementor-icon-list--layout-traditional elementor-list-item-link-full_width elementor-widget elementor-widget-icon-list"
                        data-id={"e9064d9"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-list.default"}
                      >
                        <ul className="elementor-icon-list-items">
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Completed secondary education
                              (HSC/A-Level/Equivalent).
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Selected applicants may be invited for an
                              admission interview
                            </span>
                          </li>
                          <li className="elementor-icon-list-item">
                            <span className="elementor-icon-list-icon">
                              <svg
                                aria-hidden={"true"}
                                className="e-font-icon-svg e-fas-circle"
                                viewBox="0 0 512 512"
                                xmlns="http://www.w3.org/2000/svg"
                              >
                                <path d="M256 8C119 8 8 119 8 256s111 248 248 248 248-111 248-248S393 8 256 8z" />
                              </svg>
                            </span>
                            <span className="elementor-icon-list-text">
                              Minimum GPA/grade requirements as set by the
                              university.
                            </span>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div
                    data-aos-once={"true"}
                    className="elementor-element elementor-element-73f0954 e-con-full e-flex e-con e-child"
                    data-id={"73f0954"}
                    data-element_type={"container"}
                    data-e-type={"container"}
                  >
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-49ae168 e-con-full e-flex e-con e-child"
                      data-id={"49ae168"}
                      data-element_type={"container"}
                      data-e-type={"container"}
                      data-settings={'{"background_background":"classic"}'}
                    >
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-9eac734 elementor-view-default elementor-position-block-start elementor-mobile-position-block-start elementor-widget elementor-widget-icon-box"
                        data-id={"9eac734"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-box.default"}
                      >
                        <div className="elementor-icon-box-wrapper">
                          <div className="elementor-icon-box-icon">
                            <span className="elementor-icon">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 75 64"
                              >
                                <path d="M52.925 4.80623L47.35 0.731222C46.025 -0.243741 44.25 -0.243741 42.925 0.731222L21.5375 16.3812C20.5625 17.0938 20 18.1938 20 19.4062C20 20.6188 20.5625 21.7188 21.5375 22.4313L28.8875 27.8095V38.7563C28.8875 41.5187 31.1375 43.7563 33.8875 43.7563H47.725C47.8875 42.8938 48.125 42.0563 48.4375 41.2563H33.8875C32.5125 41.2563 31.3875 40.1313 31.3875 38.7563V29.6388L42.925 38.0812C43.5875 38.5687 44.3625 38.8188 45.1375 38.8188C45.9126 38.8188 46.6876 38.5687 47.35 38.0812L58.8875 29.641V32.7188C59.7061 32.5727 60.5438 32.499 61.3875 32.5063V27.8121L65.1375 25.0688C64.876 24.2537 65.0449 18.4621 65 17.5063C65 16.3313 65.325 15.2313 65.9 14.2938L62.3375 11.6937C56.7237 9.57726 50.1734 13.675 49.1626 19.4938C48.8126 21.5039 46.8129 22.893 44.8 22.5063C42.8044 22.1733 41.391 20.1163 41.7876 18.1312C42.8875 12.1687 47.325 7.03127 52.925 4.80623Z" />
                                <path d="M72.4963 13.9855C65.2612 1.62975 47.1762 4.5946 44.2493 18.5886C43.9805 20.2016 46.3779 20.6582 46.7078 19.04C48.9863 7.95376 63.0199 4.93137 69.6436 14.1319C68.9101 14.4828 68.3156 15.0634 67.9395 15.7822L67.6 15.5313C67.6925 15.6654 67.764 15.81 67.8497 15.9475C67.6304 16.4237 67.5 16.9486 67.5 17.5063V23.7563C67.6289 28.6995 74.8708 28.7015 75 23.7562V17.5063C75 15.8775 73.95 14.5021 72.4963 13.9855Z" />
                                <path d="M7.5 52.5063C4.73755 52.5063 2.5 50.2687 2.5 47.5063V31.2563H26.25C26.9403 31.2563 27.5 30.6966 27.5 30.0063C27.5 29.316 26.9403 28.7563 26.25 28.7563H2.5V17.5063C2.5 14.7438 4.73755 12.5063 7.5 12.5063H22.6L26.025 10.0063H7.5C3.3625 10.0063 0 13.3688 0 17.5063V47.5063C0 51.6438 3.3625 55.0063 7.5 55.0063H50.6375C50 54.2313 49.45 53.3937 49 52.5063H7.5Z" />
                                <path d="M9.375 16.2563C3.59291 16.4403 3.59466 24.8233 9.37515 25.0063H20.8125C17.7939 23.2621 16.4968 19.2816 18.3625 16.2563H9.375Z" />
                                <path d="M26.25 36.2563C27.8876 36.2305 27.8973 33.7841 26.25 33.7563H8.75C7.11235 33.7821 7.10266 36.2284 8.75007 36.2563H26.25Z" />
                                <path d="M27.5 40.0063C27.5 39.3154 26.9403 38.7563 26.25 38.7563H15C14.3097 38.7563 13.75 39.3154 13.75 40.0063C13.75 40.6972 14.3097 41.2563 15 41.2563H26.25C26.9403 41.2563 27.5 40.6972 27.5 40.0063Z" />
                                <path d="M61.25 35.0063C50.1232 35.059 45.8769 49.5085 55 55.6063V62.5063C54.9722 63.4175 56.0223 64.05 56.8125 63.6187L61.25 61.4063C61.5554 61.5053 66.0403 63.9352 66.25 63.7563C66.9179 63.77 67.5137 63.1878 67.5 62.5063V55.6063C76.6277 49.5042 72.3701 35.0568 61.25 35.0063ZM67.5 52.3688C62.2428 57.9511 52.3944 54.0042 52.5001 46.2561C52.5 41.4313 56.425 37.5063 61.25 37.5063C68.9421 37.448 72.8944 46.9974 67.5 52.3688Z" />
                                <path d="M61.25 40.0063C57.804 40.0063 55 42.8102 55 46.2563C55.2937 54.5282 67.2075 54.526 67.5 46.2561C67.5 42.8103 64.696 40.0063 61.25 40.0063Z" />
                                <path d="M7.50007 50.0063H13.75C14.4403 50.0063 15 49.4472 15 48.7563C15 48.0654 14.4403 47.5063 13.75 47.5063H7.5C5.85876 47.5332 5.85564 49.9788 7.50007 50.0063Z" />
                                <path d="M18.75 47.5063C17.1088 47.5332 17.1056 49.9788 18.7501 50.0063H25C25.6903 50.0063 26.25 49.4472 26.25 48.7563C26.25 48.0654 25.6903 47.5063 25 47.5063H18.75Z" />
                                <path d="M30 47.5063C28.3588 47.5332 28.3556 49.9788 30.0001 50.0063H36.25C36.9403 50.0063 37.5 49.4472 37.5 48.7563C37.5 48.0654 36.9403 47.5063 36.25 47.5063H30Z" />
                              </svg>
                            </span>
                          </div>
                        </div>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-070905a elementor-widget elementor-widget-rs-heading"
                        data-id={"070905a"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"rs-heading.default"}
                      >
                        {" "}
                        <div className="prelements-heading default center  animate-  ">
                          {" "}
                          <div className="title-inner">
                            <h4 className="title  ">
                              Biomedical Research
                            </h4>{" "}
                          </div>{" "}
                          <div className="descripti">
                            <p>
                              KKCP provides transparent, competitive tuition
                              fees and flexible payment options, ensuring
                              high-quality education.
                            </p>{" "}
                          </div>{" "}
                        </div>
                      </div>
                    </div>
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-b05eb25 e-con-full e-flex e-con e-child"
                      data-id={"b05eb25"}
                      data-element_type={"container"}
                      data-e-type={"container"}
                      data-settings={'{"background_background":"classic"}'}
                    >
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-fab8649 elementor-view-default elementor-position-block-start elementor-mobile-position-block-start elementor-widget elementor-widget-icon-box"
                        data-id={"fab8649"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-box.default"}
                      >
                        <div className="elementor-icon-box-wrapper">
                          <div className="elementor-icon-box-icon">
                            <span className="elementor-icon">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 80 80"
                              >
                                <path d="M72.6563 47.5397H42.3179C45.6624 45.8023 48.8693 43.3686 51.9154 40.2536C52.0022 40.1635 52.0662 40.054 52.1022 39.9342C52.1381 39.8144 52.1449 39.6878 52.1221 39.5648C52.0379 39.1439 48.6933 22.3445 48.6551 22.1301L48.6933 22.0995L52.4588 22.222C53.8594 22.2833 55.0304 23.2859 55.2983 24.6559L55.4361 25.3217C54.0432 25.9187 53.209 27.4264 53.5151 28.9725C53.6376 29.6078 53.9436 30.1742 54.3876 30.6256C53.1707 33.0519 53.8901 36.2586 53.8977 36.3351C53.9819 36.7484 54.3952 37.0086 54.7932 36.9244L61.9568 35.5008C62.3624 35.4242 62.6227 34.9726 62.5538 34.567C62.5002 34.3833 62.0257 31.23 59.8138 29.4162C60.0052 28.8651 60.0586 28.2759 59.944 27.6942C59.6455 26.2094 58.3902 25.1686 56.9438 25.0767L56.7983 24.3572C56.3927 22.2908 54.6324 20.7831 52.5201 20.6912L50.7521 20.6376L56.3085 16.6808C56.5533 16.5047 56.6682 16.1986 56.6146 15.9078C56.5859 15.7626 56.5157 15.6289 56.4124 15.5229C56.3091 15.4169 56.1773 15.3432 56.0329 15.3108L26.5516 8.4992C26.4454 8.47664 26.3356 8.47587 26.2291 8.49693C26.1225 8.51798 26.0213 8.56043 25.9316 8.6217L1.28755 26.1865C1.0427 26.3626 0.927861 26.6686 0.981454 26.9595C1.01009 27.1047 1.08033 27.2384 1.18361 27.3444C1.2869 27.4504 1.41877 27.524 1.56317 27.5564L10.7168 29.6687C11.1683 31.9189 13.8165 45.2512 14.1838 47.1109C14.2297 47.3559 14.3982 47.5625 14.6354 47.662C20.054 49.9809 25.7405 51.2592 31.6108 50.7234C33.1644 53.4098 32.7741 56.0579 30.4246 58.7672H22.8858C19.3729 58.7672 16.5105 61.6219 16.5105 65.1425C16.5105 68.6554 19.3729 71.518 22.8858 71.518H64.7735C64.9218 71.5179 65.0668 71.4744 65.1905 71.3927C65.3143 71.3111 65.4114 71.195 65.4699 71.0587C65.5293 70.9242 65.5487 70.7753 65.5256 70.6301C65.5025 70.4848 65.438 70.3493 65.3397 70.2398C62.2937 66.8953 62.2247 63.6348 65.1254 60.298H72.6641C76.1847 60.2903 79.0318 57.4356 79.0318 53.915C79.0318 50.4022 76.1769 47.5397 72.6563 47.5397ZM58.4438 27.9926C58.6276 28.934 58.0152 29.8601 57.0661 30.0439C56.1247 30.2353 55.2063 29.6229 55.0151 28.674C54.8007 27.5948 55.6274 26.5845 56.7294 26.5845C57.5483 26.5845 58.2754 27.1584 58.4438 27.9926ZM3.56052 26.4467L26.5363 10.0683L54.0354 16.4208L48.2265 20.5536L35.889 20.1709C36.0957 19.6659 36.1416 19.1454 35.9885 18.6479C35.4144 16.7959 32.8276 16.5815 30.9066 17.1786C28.9702 17.7756 26.988 19.4287 27.5544 21.2654C28.1361 23.1329 30.7383 23.3244 32.6363 22.735C33.4399 22.49 34.1365 22.115 34.695 21.6634L46.1677 22.0233L31.0521 32.7917L3.56052 26.4467ZM14.3213 39.9398L12.3543 30.0439L31.0441 34.3604C31.2736 34.414 31.488 34.3604 31.6641 34.2379L47.2849 23.1022L49.2519 32.9981C39.2105 43.0854 27.4777 45.435 14.3213 39.9398ZM63.166 69.9875H28.9625V65.9081H61.5129C61.666 67.2781 62.2169 68.6405 63.166 69.9875ZM61.5129 64.3775H28.9625V60.2983H63.166C62.2169 61.6375 61.666 63.0073 61.5129 64.3775ZM66.5794 58.7675H32.376C33.3327 57.4205 33.8836 56.0504 34.0291 54.6806H66.5794V58.7675ZM66.5794 53.1498H34.0291C33.9296 52.2772 33.6694 51.3972 33.2408 50.5247C35.1543 50.2415 37.0216 49.7517 38.8585 49.0704H66.5794V53.1498Z" />
                              </svg>
                            </span>
                          </div>
                        </div>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-3d1d9ee elementor-widget elementor-widget-rs-heading"
                        data-id={"3d1d9ee"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"rs-heading.default"}
                      >
                        {" "}
                        <div className="prelements-heading default center  animate-  ">
                          {" "}
                          <div className="title-inner">
                            <h4 className="title  ">Clinical Trials</h4>{" "}
                          </div>{" "}
                          <div className="descripti">
                            <p>
                              At KKCP, we offer world-class academic programs,
                              expert faculty guidance, and innovative learning
                              opportunities.
                            </p>{" "}
                          </div>{" "}
                        </div>
                      </div>
                    </div>
                    <div
                      data-aos-once={"true"}
                      className="elementor-element elementor-element-0b2fa59 e-con-full e-flex e-con e-child"
                      data-id={"0b2fa59"}
                      data-element_type={"container"}
                      data-e-type={"container"}
                      data-settings={'{"background_background":"classic"}'}
                    >
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-534fdec elementor-view-default elementor-position-block-start elementor-mobile-position-block-start elementor-widget elementor-widget-icon-box"
                        data-id={"534fdec"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"icon-box.default"}
                      >
                        <div className="elementor-icon-box-wrapper">
                          <div className="elementor-icon-box-icon">
                            <span className="elementor-icon">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 80 80"
                              >
                                <path d="M59.0922 40.2521C58.907 40.1546 57.4259 39.4102 55.8778 39.6144L52.6578 37.1689V34.7535C53.8619 34.2374 54.8618 33.3381 55.5021 32.1953C55.7672 32.269 56.0409 32.3069 56.3159 32.308C57.1168 32.308 58.091 31.9723 58.9696 30.7395C59.6472 29.7883 59.9837 28.8126 59.9697 27.8402C59.9647 27.2916 59.7711 26.7613 59.4213 26.3386C60.9051 25.3123 63.9093 23.3654 64.9689 22.3346V28.5322C64.2921 28.6707 63.6781 29.0242 63.2185 29.5399C62.7588 30.0556 62.4781 30.7061 62.4181 31.3943C62.3397 32.2098 62.3612 33.0319 62.4821 33.8422C62.6392 34.9541 61.4609 35.6457 61.4142 35.673C61.2931 35.7409 61.1942 35.8423 61.1294 35.9651C61.0646 36.0878 61.0367 36.2267 61.049 36.365C61.0613 36.5032 61.1133 36.635 61.1987 36.7444C61.2842 36.8538 61.3995 36.9361 61.5307 36.9815C62.5534 37.3605 63.6319 37.5676 64.7222 37.5944C65.6151 37.618 66.4965 37.3899 67.2658 36.9361C67.7292 36.6411 68.1101 36.2332 68.3728 35.7508C68.6354 35.2683 68.7713 34.727 68.7675 34.1777V31.6498C68.7663 30.9503 68.5345 30.2707 68.1082 29.7161C67.6819 29.1615 67.0847 28.7629 66.4091 28.5818V19.5979C66.4081 19.5623 66.4045 19.5268 66.3984 19.4917C66.369 18.7998 66.1045 18.1386 65.6485 17.6173C61.6207 13.0851 56.825 9.29883 51.482 6.43247C50.7447 6.03905 49.922 5.83325 49.0864 5.83325C48.2508 5.83325 47.428 6.03905 46.6908 6.43247C41.3482 9.29855 36.5531 13.0845 32.5256 17.6163C32.0213 18.1947 31.752 18.9412 31.7711 19.7084C31.7901 20.4756 32.096 21.2078 32.6285 21.7605C33.8033 22.973 35.0872 24.0748 36.4639 25.0518L38.5556 26.6136C38.3299 26.9833 38.2082 27.4071 38.2034 27.8402C38.1894 28.8124 38.526 29.7879 39.2036 30.7395C40.0799 31.9699 41.049 32.3099 41.8527 32.3099C42.13 32.3088 42.4059 32.2704 42.6729 32.1955C43.2666 33.2607 44.1762 34.1154 45.2761 34.6418V37.0731L42.2656 38.526C42.0277 38.1775 41.7159 37.8857 41.3524 37.6715C40.9888 37.4572 40.5825 37.3258 40.1624 37.2866C39.4454 37.227 38.7239 37.3197 38.0452 37.5586C37.8842 37.3901 26.1949 24.1636 23.7717 21.4215C23.6447 21.2782 23.5652 21.0992 23.544 20.9089C23.5228 20.7186 23.561 20.5265 23.6534 20.3588C26.2889 15.5974 27.6363 11.647 28.3973 8.33158C29.4615 8.59636 30.4578 9.08289 31.321 9.75933C32.1841 10.4358 32.8948 11.2869 33.4063 12.2569C33.5371 12.5015 33.5803 12.7834 33.5288 13.0559C33.4773 13.3284 33.3342 13.5751 33.1232 13.7551C30.6332 15.9034 28.3675 18.2988 26.3611 20.9045C26.241 21.0562 26.1845 21.2487 26.2037 21.4413C26.2229 21.6338 26.3162 21.8114 26.464 21.9364C26.5389 21.9956 26.6249 22.0393 26.7168 22.0649C26.8088 22.0906 26.9049 22.0977 26.9997 22.0858C27.0944 22.074 27.1859 22.0434 27.2687 21.9959C27.3515 21.9484 27.424 21.8849 27.4821 21.809C29.4347 19.2698 31.6407 16.9359 34.066 14.8435C34.525 14.4507 34.8359 13.9126 34.9469 13.3188C35.058 12.7249 34.9626 12.1109 34.6765 11.5787C33.8881 10.1021 31.9663 7.51224 27.9546 6.75543C27.8586 6.73733 27.7599 6.73895 27.6645 6.7602C27.5692 6.78145 27.4791 6.82189 27.3999 6.87904C27.3207 6.9362 27.2539 7.00888 27.2037 7.09266C27.1534 7.17645 27.1208 7.26958 27.1077 7.36639C27.1058 7.38058 27.0505 7.76218 26.8935 8.45007C26.464 8.40574 26.0318 8.39364 25.6004 8.41386C25.0271 8.44046 24.4821 8.67108 24.0638 9.06413C23.6455 9.45717 23.3815 9.98676 23.3193 10.5574C23.1412 12.1919 22.5212 15.4818 20.4054 19.4397C19.5994 19.4509 18.8263 19.7614 18.2362 20.3107C17.2648 21.2149 16.3902 22.2179 15.6266 23.3033C15.3022 23.7639 15.1015 24.3 15.0436 24.8604C14.9857 25.4207 15.0726 25.9866 15.296 26.5037C13.7023 28.2 11.9708 29.7613 10.1192 31.1715C9.6553 31.5269 9.335 32.0375 9.21691 32.6098C9.09882 33.1821 9.19083 33.7778 9.47611 34.2878C9.69554 34.6728 9.95204 35.0355 10.242 35.3706C10.0256 35.5831 9.80785 35.7948 9.58884 36.0059C9.47513 36.115 9.40039 36.2585 9.37609 36.4142C9.35179 36.57 9.37926 36.7294 9.4543 36.868C10.2247 38.1398 11.2626 39.2287 12.4959 40.0592C13.7293 40.8897 15.1286 41.4419 16.5967 41.6775C17.2892 41.7753 17.9929 41.6071 18.5662 41.2066C18.8514 41.0004 19.0925 40.7393 19.2755 40.4387C19.4585 40.1381 19.5797 39.804 19.632 39.456C20.0779 36.7088 20.8451 34.0235 21.9178 31.4554C22.4481 31.5504 22.9338 31.8131 23.3035 32.205L26.3243 35.4256C26.4317 42.5672 25.4398 49.5677 22.8715 51.274C20.5431 52.822 19.4477 51.5334 19.317 51.3619C19.2076 51.188 17.8715 48.9839 17.6076 44.6195C17.6021 44.5252 17.5781 44.4328 17.5369 44.3477C17.4956 44.2627 17.438 44.1866 17.3673 44.1239C17.2965 44.0611 17.2141 44.013 17.1248 43.9822C17.0354 43.9514 16.9408 43.9386 16.8465 43.9444C16.6591 43.9637 16.4857 44.0526 16.3607 44.1935C16.2357 44.3345 16.1681 44.5172 16.1713 44.7055C15.972 48.4826 16.1096 56.828 24.4651 55.9373C25.3414 55.8439 26.2412 55.8227 27.1054 56.1193C36.4277 59.3195 35.3854 73.0891 35.3737 73.228C35.3577 73.4182 35.4179 73.607 35.5411 73.7528C35.6642 73.8987 35.8403 73.9897 36.0305 74.0057C36.0511 74.0074 36.0717 74.0082 36.0922 74.0082H70.1061C70.2956 74.0082 70.4775 73.9335 70.6123 73.8002C70.7471 73.667 70.8239 73.486 70.826 73.2964C70.9861 59.2039 67.8379 51.0064 65.169 46.611C62.293 41.8778 59.3584 40.3809 59.0922 40.2521ZM18.2085 39.2378C18.1846 39.3977 18.129 39.5512 18.045 39.6893C17.961 39.8274 17.8503 39.9474 17.7193 40.0421C17.5923 40.1352 17.4481 40.2021 17.295 40.2388C17.1418 40.2756 16.9829 40.2814 16.8275 40.256C14.4881 39.8598 12.3992 38.5572 11.0142 36.6306C12.896 34.7977 14.6707 32.8945 16.2956 30.9668L16.6313 30.5832L19.5141 33.8299C18.936 35.595 18.4994 37.4033 18.2085 39.2378ZM24.4162 31.2863C24.0452 30.8783 23.5942 30.5511 23.0912 30.3251C22.5882 30.0992 22.044 29.9793 21.4926 29.973C21.1574 29.9744 20.8253 29.909 20.5156 29.7806C20.206 29.6521 19.925 29.4633 19.6892 29.2251L16.9896 26.485C16.6879 26.1764 16.5041 25.7716 16.4703 25.3413C16.4364 24.911 16.5547 24.4825 16.8045 24.1305C17.5108 23.127 18.3197 22.1999 19.2181 21.3641C19.5604 21.0468 20.0109 20.8723 20.4776 20.8761C20.9443 20.8799 21.3919 21.0618 21.729 21.3847L24.2771 24.1543C24.9365 24.8714 25.3748 25.7637 25.5393 26.7239C25.9169 28.8819 26.157 31.0617 26.2581 33.2502L24.4162 31.2863ZM54.7451 40.5615L53.3167 42.9683L50.0529 40.4288L52.0026 38.4789L54.7451 40.5615ZM52.5759 44.2161L50.9444 46.9652L50.3975 42.5219L52.5759 44.2161ZM48.9676 39.4776L47.1491 37.6592L51.2182 35.8734V37.227L48.9676 39.4776ZM57.9478 26.9876C58.1203 27.0591 58.2677 27.1803 58.3713 27.3357C58.4748 27.4912 58.5299 27.6738 58.5296 27.8606C58.539 28.5203 58.2926 29.2078 57.7966 29.9041C57.2669 30.6476 56.6918 30.9588 56.0491 30.8471C56.0868 30.7128 56.1217 30.5753 56.152 30.4334C56.3257 29.6204 56.3906 28.7878 56.3451 27.9577C56.5144 27.6584 56.7268 27.3856 56.9754 27.148C57.1033 27.0279 57.2639 26.9485 57.437 26.92C57.6101 26.8914 57.7877 26.9149 57.9474 26.9876H57.9478ZM40.3769 29.9039C39.8812 29.2074 39.6345 28.5199 39.644 27.8606C39.6436 27.6738 39.6988 27.4911 39.8024 27.3356C39.906 27.1801 40.0534 27.0589 40.226 26.9874C40.3855 26.9148 40.5631 26.8913 40.7361 26.9198C40.9091 26.9483 41.0697 27.0276 41.1975 27.1476C41.4456 27.3844 41.6577 27.6563 41.827 27.9546C41.7818 28.7858 41.8471 29.6192 42.0212 30.4332C42.052 30.5751 42.0874 30.7121 42.124 30.8465C41.483 30.9588 40.9069 30.6482 40.3769 29.9039ZM40.9118 21.3933L41.0225 19.3979C41.0469 18.9432 41.248 18.5159 41.5828 18.2073C42.796 17.1216 44.2184 16.2956 45.7626 15.7799C47.9175 15.0443 50.2556 15.0443 52.4105 15.7799C53.9547 16.2956 55.3772 17.1216 56.5903 18.2073C56.9253 18.5159 57.1263 18.9431 57.1507 19.3979L57.2614 21.3933C55.8652 20.9551 53.1291 20.3131 49.0867 20.3131C45.0443 20.3131 42.3081 20.9553 40.9118 21.3933ZM43.4282 30.1275C42.5824 26.5868 44.6213 23.156 45.4222 21.9539C47.8489 21.5537 50.3247 21.5537 52.7514 21.9539C53.5321 23.1264 55.5285 26.5636 54.7451 30.1275C53.7273 34.7517 44.6901 35.4091 43.4282 30.1275ZM45.849 38.3956L47.8814 40.4282L44.8077 42.8196L43.0096 39.7658L45.849 38.3956ZM47.7134 42.3835L47.2485 46.965L45.5446 44.0711L47.7134 42.3835ZM49.6762 54.8651C49.6203 54.9565 49.5418 55.0319 49.4484 55.0843C49.3549 55.1366 49.2496 55.1641 49.1424 55.1641C49.0353 55.1641 48.93 55.1366 48.8365 55.0843C48.743 55.0319 48.6646 54.9565 48.6086 54.8651L38.7887 38.8578C39.1953 38.7546 39.6143 38.7082 40.0336 38.7198C40.2595 38.7407 40.4771 38.8151 40.6686 38.9367C40.86 39.0583 41.0198 39.2237 41.1348 39.4192L48.4665 51.8712C48.53 51.979 48.6205 52.0684 48.729 52.1305C48.8376 52.1927 48.9605 52.2255 49.0856 52.2257C49.2107 52.2259 49.3337 52.1935 49.4424 52.1317C49.5512 52.0699 49.642 51.9808 49.7059 51.8733L56.1407 41.0311C56.7657 41.005 57.3906 41.0874 57.9875 41.2747L49.6762 54.8651Z" />
                                <path d="M46.7424 27.6411C46.9334 27.6411 47.1165 27.5653 47.2516 27.4302C47.3866 27.2952 47.4624 27.1121 47.4624 26.9211V26.2532C47.4624 26.0622 47.3866 25.8791 47.2516 25.7441C47.1165 25.6091 46.9334 25.5332 46.7424 25.5332C46.5515 25.5332 46.3684 25.6091 46.2333 25.7441C46.0983 25.8791 46.0225 26.0622 46.0225 26.2532V26.9211C46.0225 27.1121 46.0983 27.2952 46.2333 27.4302C46.3684 27.5653 46.5515 27.6411 46.7424 27.6411ZM51.4306 27.6411C51.6215 27.6411 51.8046 27.5653 51.9397 27.4302C52.0747 27.2952 52.1505 27.1121 52.1505 26.9211V26.2532C52.1505 26.0622 52.0747 25.8791 51.9397 25.7441C51.8046 25.6091 51.6215 25.5332 51.4306 25.5332C51.2396 25.5332 51.0565 25.6091 50.9215 25.7441C50.7864 25.8791 50.7106 26.0622 50.7106 26.2532V26.9211C50.7106 27.1121 50.7864 27.2952 50.9215 27.4302C51.0565 27.5653 51.2396 27.6411 51.4306 27.6411ZM51.9162 31.1343C52.0352 30.987 52.0914 30.7988 52.0726 30.6104C52.0538 30.422 51.9616 30.2486 51.8159 30.1277C51.6702 30.0068 51.4827 29.9482 51.2941 29.9645C51.1055 29.9808 50.9309 30.0708 50.8081 30.2149C50.7258 30.3135 50.2586 30.8049 49.0768 30.8049C47.895 30.8049 47.4273 30.3135 47.3454 30.2147C47.285 30.142 47.2109 30.0818 47.1273 30.0377C47.0436 29.9935 46.9521 29.9663 46.858 29.9575C46.6678 29.9398 46.4784 29.9984 46.3314 30.1203C46.1845 30.2423 46.0919 30.4176 46.0742 30.6077C46.0565 30.7979 46.1151 30.9873 46.237 31.1343C46.5141 31.4681 47.3456 32.2451 49.0768 32.2451C50.8081 32.2451 51.6394 31.4683 51.9162 31.1343Z" />
                              </svg>
                            </span>
                          </div>
                        </div>
                      </div>
                      <div
                        data-aos-once={"true"}
                        className="elementor-element elementor-element-a60773f elementor-widget elementor-widget-rs-heading"
                        data-id={"a60773f"}
                        data-element_type={"widget"}
                        data-e-type={"widget"}
                        data-widget_type={"rs-heading.default"}
                      >
                        {" "}
                        <div className="prelements-heading default center  animate-  ">
                          {" "}
                          <div className="title-inner">
                            <h4 className="title  ">
                              Public Health Research
                            </h4>{" "}
                          </div>{" "}
                          <div className="descripti">
                            <p>
                              KKCP, student life goes beyond academics offering
                              vibrant activities cultural events clubs and
                              opportunities that personal.
                            </p>{" "}
                          </div>{" "}
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>{" "}
        </main>{" "}
        <KkcpFooter />{" "}
      </div>{" "}
      <div
        id="rs-scroll-to-top"
        className="rs-page-scroll-progress"
        data-max={"113.1"}
        data-unit={"px"}
        data-reverse={"true"}
        style={{ "--rs-page-scroll-progress": "113.1px", display: "none" }}
      >
        {" "}
        <svg className="arrowup" viewBox="0 0 24 24" width="18" height="18">
          {" "}
          <path d="M13 7.828V20h-2V7.828l-5.364 5.364-1.414-1.414L12 4l7.778 7.778-1.414 1.414L13 7.828z" />{" "}
        </svg>{" "}
        <svg className="scrollprogress" width="40" height="40">
          {" "}
          <circle
            className="progress-circle"
            cx="20"
            cy="20"
            r="18"
            strokeWidth="2"
            fill="none"
            strokeDasharray="113.1"
            strokeDashoffset="113.1"
          />{" "}
        </svg>{" "}
      </div>{" "}
      <span id="elementor-device-mode" className="elementor-screen-only"></span>
      <svg
        style={{ display: "none" }}
        className="e-font-icon-svg-symbols"
      ></svg>
    </div>
  );
}
